...
export default function FaqSection() {
  ...
  return (
    <section className="py-12 md:py-24 px-4 md:px-8 overflow-hidden bg-bg-cream">
        ...
        <div>
          {faqs.map((faq, i) => {
            ...
            return (
              <div
                key={i}
                className="border-b border-border px-2 md:px-4 bg-transparent"
              >
                <button
                  type="button"
                  onClick={() => toggle(i)}
                  aria-expanded={isOpen}
                  className="w-full flex items-center justify-between text-left py-4 md:py-5 gap-4 transition-colors duration-200"
                >
                  <span
                    className={`font-serif text-sm sm:text-base md:text-lg transition-colors duration-200 ${
                      isOpen ? "text-luxury-gold" : "text-foreground-green"
                    }`}
                  >
                    {faq.question}
                  </span>
                  <ChevronDown
                    className="h-4 w-4 shrink-0 text-muted-sage transition-transform duration-300 flex-shrink-0"
                ...
                >
                  <p className="font-sans text-sm md:text-base text-muted-sage leading-relaxed pb-4 md:pb-5">
                    {faq.answer}
                  </p>
                </div>
              </div>
